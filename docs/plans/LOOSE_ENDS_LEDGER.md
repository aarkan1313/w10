...
| **Runtime drainage subsystem (Slice 4, on-demand full-res flow bake + fact cache)** | DECIDED + spec written (`docs/superpowers/specs/2026-06-02-worldgen-runtime-drainage-design.md`), NOT built. Owner-review-pending. Data-grounded: live per-page flow too slow (6.45ms@576²); coarse shortcuts REFUTED (~800m valley-misplacement, memory `worldgen10-coarse-drainage-refuted`); decision = bake correct full-res flow off-frame per region, cache (rides M3 page-pool LRU), pages sample, evict far. Procedural-first/baking-fine (owner). | Build after owner approves the spec + after 4b.11 PART B (grammar weight field). Part of finishing Slice 4. |
