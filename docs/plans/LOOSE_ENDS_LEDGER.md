...
| **M8 visible editable terrain** (M4 edit seam's other half) | Edits are collidable-not-visible; tracked since M4. **2026-06-05: edits now have a VISUAL REVIEW surface** — `feature_review.tscn` Step 8 stamps `apply_edit` craters/mounds and shows them in the collision-field point cloud (Step 7). This visualizes the COLLISION/facts edit; making edits visible in the rendered HEIGHT (the page-producer surface) is still the open half. | After render/worldgen settled. |
| **Async/background page production** | Deferred since M3; caching solved the spike. Trigger = heavy multi-pass pages. | **TRIGGERED 2026-06-02** — per-page flow DOES blow budget (576² needs ~192 relax iters = 6.45ms). The runtime-drainage decision (on-demand off-frame full-res flow bake + fact cache) IS the async/background-production answer for the flow layer; see the Slice-4 rows below. |
| **Runtime drainage subsystem (Slice 4, on-demand full-res flow bake + fact cache)** | **LARGELY REALIZED 2026-06-05 by the region-fact producer.** The decided shape (off-frame full-res bake per region + fact cache + pages sample + evict) is exactly what `ProducerKind::RegionFact` now does ON SCREEN: an async super-region bake worker (own RD) bakes macro(GPU)+carve+condition off-frame → sliced region facts cached on a region LRU → pages sample → seam-exact, never-black. The flow/drainage runs in the GPU super-macro. (Original spec: `docs/superpowers/specs/2026-06-02-worldgen-runtime-drainage-design.md`.) Remaining: the owner VISUAL A/B + production region_size_m + outer-border seam (k-knob). | Mostly DONE; see STATUS top + `WG10_HANDOFF_2026-06-05_REGION_FACT_ON_SCREEN.md`. |
| **Exact operator-squaring (path-doubling) GPU flow solver** | PARKED as a future optimization. Mathematically EXACT (matches the f64 MFD oracle to 1.7e-13) in ~8 log-steps vs ~192 relax iters — but its GPU realization densifies connectivity (growing per-cell ancestor buffers; jump-flooding does NOT apply to a weighted-sum DAG), so heavyweight + high build-risk + unproven cheaper than 192 simple passes. Memory `worldgen10-coarse-drainage-refuted`. | If the per-region drainage bake cost ever becomes the bottleneck (it makes flow O(log N) not O(N)). Only worth the build complexity then. |
