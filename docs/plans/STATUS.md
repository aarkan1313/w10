...
**Phase:** M0 + M1 (bedrock/grammar/height) + first DEM pack + M2 GPU parity ALL green. **M3 render pipeline: leaves (pool/policies/streamer/ring_geometry/page_compute) proven; the PRESENTATION layer is being rebuilt via the prove-one-at-a-time RESET** (`proving_ground.tscn`, owner-flown). Steps 1–6 owner-confirmed (one page → two-page seam → static 3×3 → streamer-driven 3×3 → coarse never-black blanket → geomorph). Remaining: **Step 7 (3rd level + p99<6 ms at ~1000 m/s = M3 acceptance)**, then fold the rebuilt presentation back into `Wg10TerrainView`/`Wg10ClipmapRings` + reconcile the m3 gates, then M4. The visible "squareness/lines" are extreme DEM data (M6/M7 content fix), not a render defect.
