...
Last updated: 2026-05-29 (**M3 RENDER-LAYER RESET in progress — owner-driven, prove-one-thing-at-a-time.** The post-slice-8 multi-level render looked "a mess" under a real fly (overlapping sheets, seams, switching) because slices 4→8 were stacked without proving the surface was continuous LIVE; the automated gates proved properties (p99, never-black, data-seam=0) but never *perceptual continuity in a flown perspective POV*. Decision: keep the proven CPU/GPU leaves (pool, page_policy, schedule_policy, streamer, ring_geometry, page_compute — modularity verified, deps clean one-directional) and rebuild ONLY the presentation (`Wg10TerrainView` + `Wg10ClipmapRings` + `ring_displace.gdshader`) one step at a time in a new `proving_ground.tscn`, owner-flown + approved before each next step. See `docs/plans/COMPONENT_INVENTORY.md` for the full inventory + step plan. **Real bugs found & fixed by this process (each owner-confirmed):** (1) **page sampler defaulted to REPEAT** wrap — tile-edge vertices at uv=1 wrapped to the page's opposite edge → seams at EVERY tile boundary (this was the dominant "overlapping sheets"); fix = `filter_linear, repeat_disable` (clamp-to-edge) in the shader. (2) **velocity lead was unit-wrong + unclamped** — `lead_frames` × m/s velocity gave up to 64 km of lead at sprint, flying the 3×3 off the camera (pop-in, terrain-lag-under-you, churn); fix = renamed `lead_seconds`, CLAMPED in `SchedulePolicy::coverage_center` to ±(radius−0.5)·span so the camera is ALWAYS in its ring at any speed; view reads the clamped centre from the streamer (no desync). (3) the step-5 "LOD line" was the morph being OFF (each tile bound its own page as the morph target = nothing to blend); step 6 wires each fine tile to the REAL coarse parent page + blends. Steps PROVEN (owner-flown): 1 one page · 2 two-page seam · 3 static 3×3 · 4 streamer-driven 3×3 · 5 coarse never-black blanket · 6 geomorph · 7 3 levels + full speed (acceptance probe p99=1.63 ms, 0 holes). **FOLD-BACK DONE:** the proven model is now in the REAL `Wg10TerrainView` + `Wg10ClipmapRings` (every level draws its full 3×3; an unready tile is HIDDEN so the coarser full 3×3 underneath shows through = never-black; a resident tile samples its own page by world UV and geomorphs toward its REAL PARENT page if not coarsest; clamp-to-edge sampler + clamped led centre carry over). Added `Wg10ClipmapRings::set_tile_visible`. **ALL gates green on the rebuilt path: m3 6/6 (accept p99=3.94 ms<6, continuity seam=0/no-shimmer/no-banding, view surrounds+zero-compute, stream never-black), gpu 2/2, fast 5/5, cargo 103.** `m3_review.tscn` loads + renders clean on the rebuilt path. **M3 needs only the owner's acceptance fly of `m3_review.tscn` (now the REAL components).** The "blue squares/hard lines" are EXTREME DEM DATA (M6/M7 content fix), not a render bug. FIXED post-fold-back: tiles vanishing on rotation + a chunk blinking in/out on slow creep were the SAME frustum-cull bug (flat meshes + GPU displacement, no custom AABB) → `Wg10ClipmapRings` now sets a tall custom AABB per tile (gates still green, p99=1.87 ms). After the fly: retire the proving ground, then M4.
