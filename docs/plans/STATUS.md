...
**Phase:** M0 + M1 (bedrock/grammar/height) + first DEM pack + M2 GPU parity + **M3 render pipeline ALL green & structurally DONE** (reset rebuilt the presentation prove-one-at-a-time, folded into the real `Wg10TerrainView`/`Wg10ClipmapRings`; m3 6/6, gpu 2/2, fast 5/5, cargo 103; owner-flown). **NEXT: M4 — Facts API + adaptable edit seam** (brainstormed 2026-05-30; spec being written): package the parity-gated base height as the drop-in `Wg10Facts` + `get_collision_field` (Jolt) + a `clamp(base + pluggable edit-provider, bedrock, ceiling)` seam with a circular-stamps provider, built as 4 slices (CPU seam → stamps+bedrock → sparse collision → GPU bulk bake). Then M5 detail → M6 biomes/materials → M7 erosion. The "squareness/LOD-pop" remaining are TEST-SCALE + CONTENT (M5–M7), not render defects. **Owner's acceptance fly of `m3_review.tscn` (5 levels + fog) still welcome as the final §7.3 sign-off, but render gates are green.** The proving-ground scene stays for now (debug scaffolding, useful for M4/LOD tuning); retire it once M4 lands.
