...
mountain payload beside the live producer as a fact/material reference. The
contract report can see pass-network routes, route carving, page-stable
conditioning, corridor facts, and material hints from that reference, and the
renderer can use the bound reference for material page presentation. Height is
still generated by the live seam-safe page recipe, so
`height_consumes_world_layer_facts=false` and full contract satisfaction remains
false.
...
4. **Thread facts into rendering.**
   ...
   mountain world-layer payload as a separate fact/material reference. Its
   contract report exposes bound pass-network, carving, conditioning, corridor,
   and material-hint facts, and its page reports expose those facts over runtime
   pages. This remains a bridge only: live height does not consume the bound
   facts yet.
...
- `review_runtime` now also proves live `MOUNTAIN/network_ref` binds the
  ...
  proves the remaining gap is explicit:
  `height_consumes_world_layer_facts=false` and
...
- Latest bridge proof after rebuild: `cargo test -p wg10_terrain --lib` =
  ...
  motion p99/max is 25.059/46.348 ms and render p99 is 0.247 ms.
