"""Export the RAINFOREST recipe-level parity fixture for the Rust biome-recipe port.

Mirrors `tools/dem_pack/export_recipe_fixtures.py` (the MOUNTAIN template) exactly,
but runs the REAL `rainforest_synthesis.generate(apron_px > 0)` seam-safe path.
...
The Rust side (`recipes_rainforest.rs::rainforest_seamsafe`) rebuilds the SAME padded
...
CORE small (24x24) so JSON is compact; APRON real (RAINFOREST_APRON_PX = 160 ->
...
Run from repo root:  python tools/dem_pack/export_recipe_rainforest_fixture.py
Writes:              tools/dem_pack/fixtures/recipe_rainforest_fixture.json
"""
...
import rainforest_synthesis as rs  # noqa: E402
...
def main():
    ...
    core_n = 24
    apron_px = rs.RAINFOREST_APRON_PX  # 160 -> padded 344x344
    feature_span_m = 90_000.0  # fixed constant shared by adjacent windows
    style = rs.STYLES[0]  # humid_dissected_hills
    ...
    out_path = os.path.join(out_dir, "recipe_rainforest_fixture.json")

    doc = {
        "generator_version": GENERATOR_VERSION,
        "source": (
            "rainforest_seamsafe <- tools/dem_pack/rainforest_synthesis.py::generate("
            "apron_px=RAINFOREST_APRON_PX) seam-safe path; "
            "core-cropped height is the parity oracle for "
            "wg-10/rust/src/recipes_rainforest.rs::rainforest_seamsafe"
        ),
        "note": (
