...
def test_mountain_generate_seam_exact():
    """Adjacent apron-padded windows must agree bit-exactly at their shared border.
    ...
    Setup:
      ...
      - After generate(apron_px=APRON), height is cropped to the core; assert the shared
        border column matches bit-exactly.
    """
    N = 64
    S = 60_000.0
    X0 = 10_000.0
    ...
    # The shared border: A's rightmost column == B's leftmost column
    border_delta = float(np.max(np.abs(a_core[:, -1] - b_core[:, 0])))
    print(f"Seam border delta (max |A[:,-1] - B[:,0]|) = {border_delta:.6e}")
    assert border_delta == 0.0, (
        f"Seam NOT bit-exact: max border delta = {border_delta:.6e}"
    )
