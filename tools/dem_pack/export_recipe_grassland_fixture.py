"""Export the GRASSLAND recipe-level parity fixture for the Rust biome-recipe port.

Mirrors `tools/dem_pack/export_recipe_fixtures.py` (the MOUNTAIN template) exactly,
but runs the REAL `grassland_synthesis.generate(apron_px > 0)` seam-safe path.
...
The Rust side (`recipes_grassland.rs::grassland_seamsafe`) rebuilds the SAME padded
...
CORE small (24x24) so JSON is compact; APRON real (GRASSLAND_APRON_PX = 160 ->
...
Run from repo root:  python tools/dem_pack/export_recipe_grassland_fixture.py
Writes:              tools/dem_pack/fixtures/recipe_grassland_fixture.json
"""
...
import grassland_synthesis as gs  # noqa: E402
...
def main():
    ...
    core_n = 24
    apron_px = gs.GRASSLAND_APRON_PX  # 160 -> padded 344x344
    feature_span_m = 90_000.0  # fixed constant shared by adjacent windows
    style = gs.STYLES[0]  # rolling_prairie
    ...
    out_path = os.path.join(out_dir, "recipe_grassland_fixture.json")

    doc = {
        "generator_version": GENERATOR_VERSION,
        "source": (
            "grassland_seamsafe <- tools/dem_pack/grassland_synthesis.py::generate("
            "apron_px=GRASSLAND_APRON_PX) seam-safe path; "
            "core-cropped height is the parity oracle for "
            "wg-10/rust/src/recipes_grassland.rs::grassland_seamsafe"
        ),
        "note": (
