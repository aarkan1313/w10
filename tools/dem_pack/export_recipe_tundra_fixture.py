"""Export TUNDRA recipe-level parity fixture for the Rust biome-recipe port.

Parity oracle for `wg-10/rust/src/recipes_tundra.rs`. Runs the REAL Python
recipe (`tools/dem_pack/tundra_synthesis.py::generate`) in its SEAM-SAFE mode
...
Format matches export_recipe_fixtures.py (recipe_fixtures/v1):

    {
      ...
      "records": [
        {
          "recipe": "tundra_seamsafe",
          "seed": <int>,
          "feature_span_m": <float>,
          "apron_px": <int>,
          "style_key": "arctic_plain",   # STYLES[0]
...
Run from repo root:  python tools/dem_pack/export_recipe_tundra_fixture.py
Writes:              tools/dem_pack/fixtures/recipe_tundra_fixture.json
"""
...
import tundra_synthesis as ts  # noqa: E402
...
def main():
    ...
    # CORE small so JSON is compact; APRON real (TUNDRA_APRON_PX = 160).
    core_n = 24
    apron_px = ts.TUNDRA_APRON_PX  # 160 -> padded 344x344
    feature_span_m = 90_000.0  # fixed constant shared by adjacent windows
    style = ts.STYLES[0]  # arctic_plain (the reference style)
    ...
    out_path = os.path.join(out_dir, "recipe_tundra_fixture.json")

    doc = {
        "generator_version": GENERATOR_VERSION,
        "source": (
            "tundra_seamsafe <- tools/dem_pack/tundra_synthesis.py::generate("
            "apron_px=TUNDRA_APRON_PX) seam-safe path; "
            "core-cropped height is the parity oracle for "
            "wg-10/rust/src/recipes_tundra.rs::tundra_seamsafe"
        ),
        "note": (
