"""Export the DESERT recipe-level parity fixture for the Rust biome-recipe port.

Mirrors `tools/dem_pack/export_recipe_fixtures.py` (the MOUNTAIN template) exactly,
but runs the REAL `desert_synthesis.generate(apron_px > 0)` seam-safe path.
...
The Rust side (`recipes_desert.rs::desert_seamsafe`) rebuilds the SAME padded
...
CORE small (24x24) so JSON is compact; APRON real (DESERT_APRON_PX = 160 ->
...
Run from repo root:  python tools/dem_pack/export_recipe_desert_fixture.py
Writes:              tools/dem_pack/fixtures/recipe_desert_fixture.json
"""
...
import desert_synthesis as ds  # noqa: E402
...
def main():
    ...
    core_n = 24
    apron_px = ds.DESERT_APRON_PX  # 160 -> padded 344x344
    feature_span_m = 90_000.0  # fixed constant shared by adjacent windows
    style = ds.STYLES[0]  # dune_sea
    ...
    out_path = os.path.join(out_dir, "recipe_desert_fixture.json")

    doc = {
        "generator_version": GENERATOR_VERSION,
        "source": (
            "desert_seamsafe <- tools/dem_pack/desert_synthesis.py::generate("
            "apron_px=DESERT_APRON_PX) seam-safe path; "
            "core-cropped height is the parity oracle for "
            "wg-10/rust/src/recipes_desert.rs::desert_seamsafe"
        ),
        "note": (
