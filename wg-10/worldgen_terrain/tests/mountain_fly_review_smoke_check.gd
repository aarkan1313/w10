...
func _expect_mode_switch(
	...
	if mode == "REFERENCE":
		...
		_expect(bool(reference.get("has_material_hints", false)), "REFERENCE expected material hint facts", errs)
		_expect(float(reference.get("floor_hint_frac", 0.0)) > 0.0, "REFERENCE expected floor hint coverage", errs)
		_expect(float(reference.get("rock_hint_frac", 0.0)) > 0.0, "REFERENCE expected rock hint coverage", errs)
		...
		_expect(bool(reference.get("has_conditioning_stats", false)), "REFERENCE expected conditioning facts", errs)
		_expect(float(reference.get("conditioning_source_ptp", 0.0)) > 0.0, "REFERENCE expected source conditioning span", errs)
		_expect(float(reference.get("conditioning_p95", 0.0)) > float(reference.get("conditioning_p05", 0.0)), "REFERENCE expected ordered conditioning percentiles", errs)
		_expect(float(reference.get("conditioning_ptp", 0.0)) > 0.0, "REFERENCE expected conditioned height span", errs)
		var center_page: Dictionary = snapshot.get("static_reference_center_page", {})
		_expect(bool(center_page.get("has_corridor", false)), "REFERENCE center page expected corridor facts", errs)
		_expect(int(center_page.get("samples_px", 0)) == 17, "REFERENCE center page expected 17 samples", errs)
		_expect(float(center_page.get("corridor_frac", -1.0)) >= 0.0, "REFERENCE center page expected corridor fraction", errs)
		_expect(bool(center_page.get("has_material_hints", false)), "REFERENCE center page expected material hint facts", errs)
		_expect(float(center_page.get("floor_hint_mean", -1.0)) >= 0.0, "REFERENCE center page expected floor hint mean", errs)
		_expect(float(center_page.get("rock_hint_mean", -1.0)) >= 0.0, "REFERENCE center page expected rock hint mean", errs)
