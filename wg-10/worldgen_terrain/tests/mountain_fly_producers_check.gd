...
func _run() -> int:
	...
	_expect(absf(float(producers.view_relief_scale(0.25)) - 0.25) < 0.001, "default view relief scale should pass through", errs)
	...
	_expect(not bool(producers.is_reference()), "default should not be REFERENCE", errs)
