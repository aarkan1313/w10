...
func _run() -> int:
	...
	_expect(absf(float(producers.source_scale()) - 1.0) < 0.000001, "mountain helper source scale should stay identity before runtime tile binding", errs)
	_expect(absf(float(producers.source_offset_x_m())) < 0.001, "mountain helper source x offset should stay identity before runtime tile binding", errs)
	_expect(absf(float(producers.source_offset_z_m())) < 0.001, "mountain helper source z offset should stay identity before runtime tile binding", errs)
	...
	_expect(absf(float(producers.view_relief_scale(0.25)) - 1.0) < 0.001, "WORLD preview relief scale should follow accepted reference height", errs)
	_expect(absf(float(producers.view_relief_ref(1700.0, 0.25)) - 20000.0) < 0.001, "WORLD relief ref should follow clamped relief and reference preview scale", errs)
	...
	_expect_taxonomy(producers, "world_composition_diagnostic", "diagnostic_not_owner_accepted", "reference height", errs)
	producers.cycle_mode()
