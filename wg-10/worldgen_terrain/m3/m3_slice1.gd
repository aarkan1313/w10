...
func _run() -> int:
	# --- guard: native class must be registered ---
	if not ClassDB.class_exists("Wg10PageCompute"):
		push_error("[wg10-m3-slice1] Wg10PageCompute not registered — run WINDOWED (not headless)")
		return 1
	...
	if load_err != "":
		push_error("[wg10-m3-slice1] load_pack_dir failed: %s" % load_err)
		return 1

	var tex = pc.call("compute_page", glsl_os, 0.0, 0.0, WORLD_SPAN, PAGE_PX, SEED)
	if tex == null:
		push_error("[wg10-m3-slice1] compute_page returned null — check height_page.glsl / Wg10PageCompute")
		return 1
