...
#[derive(Deserialize)]
pub(super) struct StaticRuntimeTilePayload {
    generator_version: String,
    source_scope: String,
    chunk_count: usize,
    chunk_n: usize,
...
#[derive(Deserialize)]
struct StaticRuntimeTile {
    generator_version: String,
    source_scope: String,
    chunk_count: usize,
    chunk_n: usize,
    field_n: usize,
    field_origin_x_m: f64,
    field_origin_z_m: f64,
    field_span_m: f64,
    height_scale_m: f64,
    pass_network: Option<StaticPassNetwork>,
    stats: Option<StaticConditioningStats>,
...
#[derive(Deserialize)]
struct StaticRuntimeTileFields {
    height: Vec<f32>,
    corridor: Option<Vec<i64>>,
    low_pass_hint: Option<Vec<f32>>,
    floor_hint: Option<Vec<f32>>,
...
impl StaticHeightRuntime {
    ...
    fn from_runtime_tile_payload(payload: StaticRuntimeTilePayload) -> Result<Self, String> {
        if payload.chunk_count == 0 {
...
mod tests {
    ...
    fn payload_contract_metadata_and_corridor_are_preserved() {
        let rt = StaticHeightRuntime::from_payload(StaticPayload::Review(
            one_chunk_review_payload(Some(vec![1, 0, 1, 0])),
        ))
