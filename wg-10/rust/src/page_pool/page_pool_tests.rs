...
use crate::pack::{GrammarConstants, Pack};
...
fn reset_clears_all_configured_state_no_half_configured_residue() {
    ...
    let mut slot_tex: Vec<Option<Rid>> = vec![Some(Rid::new(1)), None, Some(Rid::new(2)), None];
    ...
    assert!(
        ...
        "slot_material_tex must be empty"
    );
    assert!(
        slot_material_wrap.is_empty(),
        "slot_material_wrap must be empty"
    );
    ...
    let guard_passes =
        policy.is_some() && pack.is_some() && pack_buffers.is_some() && glsl_source.is_some();
    assert!(
        !guard_passes,
        "acquire guard must FAIL after reset (unconfigured)"
    );
    assert_eq!(
        guard_passes,
        compute_ctx.is_some(),
...
fn configured_predicate_requires_all_four_config_options() {
    ...
    assert!(
        guard(true, true, true, true, true),
        "all Some => configured"
    );
    // Any single missing option => NOT configured (guard returns early).
    assert!(
        !guard(false, true, true, true, true),
        "missing policy => unconfigured"
    );
    assert!(
        !guard(true, false, true, true, true),
        "missing pack => unconfigured"
    );
    assert!(
        !guard(true, true, false, true, true),
        "missing pack_buffers => unconfigured"
    );
    assert!(
        !guard(true, true, true, false, true),
        "missing glsl_source => unconfigured"
    );
    assert!(
        !guard(true, true, true, true, false),
        "missing compute_ctx => unconfigured"
    );
    assert!(
        !guard(false, false, false, false, false),
        "all None => unconfigured"
    );
}
...
fn fresh_pool_not_configured_either_path() {
    ...
    assert!(
        !is_configured(true, false, false, false, false, false, false, false),
        "policy alone => unconfigured"
    );
    // Legacy path fully built => configured.
    assert!(
        is_configured(true, true, true, true, true, false, false, false),
        "full legacy => configured"
    );
    // Legacy path missing its compute_ctx => unconfigured (the F7 hazard, now guarded).
    assert!(
        !is_configured(true, true, true, true, false, false, false, false),
        "legacy w/o compute_ctx => unconfigured"
    );
    // Biome path => configured with ONLY policy + biome_ctx (no pack/glsl).
    assert!(
        is_configured(true, false, false, false, false, true, false, false),
        "biome path => configured"
    );
    // Biome ctx present but NO policy => unconfigured (policy is mandatory on both paths).
    assert!(
        !is_configured(false, false, false, false, false, true, false, false),
        "biome ctx w/o policy => unconfigured"
    );
    assert!(
        is_configured(true, false, false, false, false, false, true, false),
        "biome world => configured"
    );
    assert!(
        !is_configured(false, false, false, false, false, false, true, false),
        "biome world w/o policy => unconfigured"
    );
    assert!(
        is_configured(true, false, false, false, false, false, false, true),
        "static reference => configured"
    );
    assert!(
        !is_configured(false, false, false, false, false, false, false, true),
        "static reference w/o policy => unconfigured"
    );
}
